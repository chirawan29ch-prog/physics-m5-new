...
function TeacherScores({students,setStudents}){
  ...
  function doAdd(){
    ...
    if(targetMode==="all"){
      setStudents(prev=>prev.map(s=>({
        ...s,
        xp:s.xp+Number(xpAmt),
        xpLog:[...(s.xpLog||[]),logEntry]
      })));
      toast(`✅ เพิ่ม ${xpAmt} XP จาก "${activityName}" ให้นักเรียนทั้งหมด ${students.length} คน!`);
    } else {
      const name=students.find(s=>s.id===selStu)?.name||"";
      setStudents(prev=>prev.map(s=>s.id===selStu?{
        ...s,
        xp:s.xp+Number(xpAmt),
        xpLog:[...(s.xpLog||[]),logEntry]
      }:s));
  ...
  // รวบรวมกิจกรรมทั้งหมดจาก xpLog ของทุกคน
  const allActivities=useMemo(()=>{
    const map={};
    students.forEach(s=>{
      (s.xpLog||[]).forEach(log=>{
        ...
        <>
          <div className="card card-gold" style={{marginBottom:20}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                  {[["single","👤 รายบุคคล"],["all","👥 ทั้งห้อง"]].map(([m,l])=>(
                    <button key={m} onClick={()=>{setTargetMode(m);setSelStu("");}} className="btn"
                      style={{background:targetMode===m?"rgba(232,188,85,.18)":"rgba(255,255,255,.05)",
                        border:`1px solid ${targetMode===m?"rgba(232,188,85,.6)":"var(--border)"}`,
                        color:targetMode===m?"var(--gold)":"var(--muted2)",
                        borderRadius:6,padding:"10px 8px",fontSize:13,
                        fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,letterSpacing:.5}}>
                      {l}
                    </button>
                  ))}
            ...
            <button className="btn btn-gold" onClick={doAdd}
              style={{width:"100%",fontSize:17,padding:"15px 0",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
              <span>➕</span>
              {targetMode==="all"?`เพิ่ม XP ให้ทุกคน (${students.length} คน)`:"เพิ่ม XP"}
            </button>
          </div>
        ...
        <div>
          {allActivities.length===0?(
            <div className="card" style={{textAlign:"center",padding:48}}>
              <div style={{fontSize:40,marginBottom:12}}>📊</div>
              <div className="cond" style={{fontSize:20,color:"var(--muted)"}}>ยังไม่มีการให้คะแนน</div>
              <div style={{fontSize:13,color:"var(--muted)",marginTop:8}}>กลับไปที่แท็บ "เพิ่ม XP" เพื่อเริ่มให้คะแนน</div>
            </div>
          ):(
            allActivities.map((act,ai)=>{
              const sortedStudents=[...students].sort((a,b)=>b.xp-a.xp);
                ...
                <div key={ai} className="card" style={{marginBottom:16,borderColor:"rgba(232,188,85,.35)"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:8}}>
                    <div>
                      <div className="cond" style={{fontSize:20,fontWeight:700,color:"var(--gold2)"}}>{act.name}</div>
                      <div style={{display:"flex",gap:8,marginTop:6}}>
                        <span className="badge" style={{background:"rgba(94,200,126,.14)",border:"1px solid rgba(94,200,126,.4)",color:"var(--green)"}}>
                          ✓ ได้รับ {receivedCount} คน
                        </span>
                        <span className="badge" style={{background:"rgba(232,96,96,.1)",border:"1px solid rgba(232,96,96,.28)",color:"var(--red)"}}>
                          — ยังไม่ได้ {students.length-receivedCount} คน
                        </span>
                        <span className="mono" style={{fontSize:11,color:"var(--gold)",padding:"3px 8px"}}>
                          รวม {totalGiven.toLocaleString()} XP
                        </span>
                      </div>
                    </div>
                  </div>
